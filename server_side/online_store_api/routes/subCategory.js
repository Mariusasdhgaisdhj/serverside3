...
router.delete('/:id', asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Update column name if needed; assuming 'subcategory_id' based on common naming
        // Run in Supabase SQL: SELECT column_name FROM information_schema.columns WHERE table_name = 'brands' AND column_name LIKE '%sub%';
        console.log('Checking brands...');
        const { count: brandCount, error: brandError } = await supabase
            .from('brands')
            .select('*', { count: 'exact', head: true })
            .eq('subcategory_id', subCategoryID);  // <-- Confirm this column name in your brands table
